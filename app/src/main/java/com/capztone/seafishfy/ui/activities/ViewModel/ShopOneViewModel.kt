...
class ShopOneViewModel : ViewModel() {
    ...
    private suspend fun retrieveShopData(shopName: String, userLanguage: String) {
        try {
            ...
            if (snapshot.exists()) {
                retrieveMenuItems(shopRef.child("menu"), _menuItems, userLanguage)
                retrieveMenuItems(shopRef.child("menu1"), _menu1Items, userLanguage)
                retrieveMenuItems(shopRef.child("menu2"), _menu2Items, userLanguage)
                retrieveDiscountItems(shopRef.child("discount"),_discountItems,userLanguage)
            }
        } catch (e: Exception) {
            // Handle error
        }
